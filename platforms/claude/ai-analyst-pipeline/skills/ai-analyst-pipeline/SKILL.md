...
- Use the `Overview`, `Drivers`, `Scenario`, `Evidence` tab contract. Keep `Scenario` when the data supports what-if analysis, model threshold comparison, top-N/capacity tradeoffs, or segment/rule comparison such as target size versus observed event rate. Remove `Scenario` only when no defensible scenario or rule comparison can be made from source-file evidence.
- Replace placeholders with values supported by `04_kpi_summary.md` and `05_analysis_report.md`.
- Prefer numeric values from `04_metrics.json` when available, and ensure dashboard KPI values match it.
- Write all visible dashboard text in Korean by default. Keep source column names, source values, formulas, file names, and proper nouns unchanged when accuracy requires it.
- Keep KPI numbers black; use color for labels or visual marks only.
- Use SVG/Canvas charts only.
- Do not use external chart libraries, external fonts, or CDN assets.
- Scale every SVG chart against the actual maximum plotted value, not against a business baseline. No SVG geometry may leave the `viewBox`; reject or fix any `<rect>` with negative `x`/`y`, negative width/height, or bars that extend beyond the plot area.
- Include units, calculation bases, and sample-size warnings where applicable.
